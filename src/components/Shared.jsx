...
export function ProgressBar({ current, total, color = T.blue }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
      <span style={{ color: T.muted, fontSize: 13, minWidth: 60 }}>
        {current} / {total}
      </span>
...
export function ResultScreen({ correct, total, onRetry, onBack }) {
  ...
  const msg =
    pct >= 85 ? 'Ausgezeichnet! 🏆' : pct >= 70 ? 'Sehr gut! 💪' : pct >= 50 ? 'Solide Basis 📚' : 'Weiter üben! 🔁'
  useEffect(() => {
    const h = (e) => {
  ...
  return (
    <div style={{ textAlign: 'center', padding: '48px 0' }}>
      ...
      <div style={{ color: T.muted, fontSize: 16, marginBottom: 8 }}>
        ...
        {options.map((opt) => (
          <button
            ...
            style={{
              ...
              borderRadius: 8,
              color: value === opt.value ? T.blue : T.text,
              cursor: 'pointer',
              padding: '8px 16px',
              fontSize: 14,
            }}
...
export function useSettingsKeyboard(rows, onStart, onBack, active) {
  ...
  useEffect(() => {
    ...
    const h = (e) => {
      ...
      if (e.key === 'ArrowRight' && r < maxRow) {
        nc = Math.min(c + 1, rows[r].length - 1)
        moved = true
      } else if (e.key === 'ArrowLeft' && r < maxRow) {
        nc = Math.max(c - 1, 0)
        moved = true
...
export function ScoreBar({ score, total, color }) {
  ...
  return (
    <span style={{ color, fontSize: 14 }}>
      {score}/{total} <span style={{ color: T.muted }}>({pct}%)</span>
    </span>
  )
}
