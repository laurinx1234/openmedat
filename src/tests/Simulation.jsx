...
function getOptionText(q, i) {
  if (q.opts) {
    const o = q.opts[i]
    if (o === 'keine' || o === undefined) return 'Keine Antwort ist richtig.'
    if (typeof o === 'object' && o !== null) {
      if (o.shape) return o.shape.label || 'Figur'
      return 'Keine der Figuren ist richtig.'
    }
...
function ReviewPanel({ categories, onBack }) {
  ...
  const ans = cat.answers[qi]
  const correct = ans === q.correctIdx
  useEffect(() => {
    const h = (e) => {
      if (e.key === 'ArrowRight') {
        e.preventDefault()
        setQi((qi) => Math.min(qi + 1, cat.questions.length - 1))
      } else if (e.key === 'ArrowLeft') {
        e.preventDefault()
        setQi((qi) => Math.max(qi - 1, 0))
      } else if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        e.preventDefault()
        setCi((ci) => {
          const nxt = e.key === 'ArrowDown' ? Math.min(ci + 1, categories.length - 1) : Math.max(ci - 1, 0)
          return nxt
        })
        setQi(0)
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '20px 20px' }}>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
        {categories.map((c, i) => (
          <button
            ...
            style={{
              background: ci === i ? `${c.color}25` : T.surf2,
              border: `1px solid ${ci === i ? c.color : T.border}`,
              borderRadius: 8,
              color: ci === i ? c.color : T.text,
              cursor: 'pointer',
              padding: '8px 14px',
              fontSize: 13,
              display: 'flex',
          ...
          >
            ...
            <span style={{ color: T.muted, fontSize: 11 }}>
              ({c.answers.filter((a) => a !== null).length}/{c.questions.length})
            </span>
          </button>
        ))}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        <button
          onClick={() => setQi((q) => Math.max(q - 1, 0))}
          disabled={qi === 0}
          style={{
            background: 'none',
            border: 'none',
            color: qi === 0 ? T.border : T.text,
        ...
        <span style={{ color: T.text, fontSize: 14 }}>
          {qi + 1} / {cat.questions.length}
        </span>
        <button
          onClick={() => setQi((q) => Math.min(q + 1, cat.questions.length - 1))}
          disabled={qi === cat.questions.length - 1}
          style={{
            background: 'none',
            border: 'none',
            color: qi === cat.questions.length - 1 ? T.border : T.text,
            cursor: qi === cat.questions.length - 1 ? 'default' : 'pointer',
            fontSize: 18,
          }}
        >
          ►
        ...
        <div style={{ flex: 1 }} />
        <span style={{ color: correct ? T.green : T.red, fontWeight: 'bold', fontSize: 13 }}>
          {ans === null ? 'Nicht beantwortet' : correct ? '✓ Richtig' : '✗ Falsch'}
        </span>
      </div>
      <Card style={{ marginBottom: 12 }}>
        {q.p1 && q.p2 ? (
          ...
          <div style={{ fontSize: 17, color: T.text, marginBottom: q.word ? 12 : 0 }}>
            {getQuestionPrompt(q, cat.key) || 'Frage'}
          </div>
        )}
        {q.word && (
          <div style={{ color: T.muted, fontSize: 15, marginTop: 8 }}>
            Wort:{' '}
            <span style={{ color: cat.color, fontWeight: 'bold', fontSize: 20, letterSpacing: 4 }}>
              {q.word.toUpperCase()}
            </span>
          </div>
        )}
      ...
      <Card>
        {Array.from(
          { length: q.opts ? q.opts.length : q.options ? q.options.length : q.choices ? q.choices.length : 0 },
          (_, i) => {
            ...
            return (
              ...
              >
                <span style={{ color: T.yellow, minWidth: 22, fontWeight: 'bold', flexShrink: 0 }}>
                  {['A', 'B', 'C', 'D', 'E'][i]}
                </span>
                {o && o.shape ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1 }}>
                    <FigAnswerSVG shape={o.shape} size={48} />
                    <span>{o.shape.label}</span>
                  </div>
                ) : o && typeof o === 'object' && o !== null && o.shape === null ? (
                  <span style={{ flex: 1, color: T.muted }}>Keine der Figuren ist richtig.</span>
                ) : (
                  <span style={{ flex: 1 }}>{getOptionText(q, i)}</span>
                )}
                {isCorrect && isUserPick && (
                  <span style={{ color: T.green, fontWeight: 'bold', marginLeft: 'auto', flexShrink: 0 }}>
                    ✓ Richtig
                  </span>
                )}
                {isCorrect && !isUserPick && (
                ...
                {isUserPick && !isCorrect && (
                  <span style={{ color: T.red, fontWeight: 'bold', marginLeft: 'auto', flexShrink: 0 }}>
                    ✗ Deine Wahl
                  </span>
                )}
              </div>
...
function FigAnswerSVG({ shape, size = 60 }) {
  ...
  if (shape.isPlaceholder) {
    if (shape.id === 'ph_tri')
      return (
        <svg viewBox={`0 0 ${s} ${s}`} width={s} height={s}>
          <polygon
            points={`${cx},${s * 0.08} ${s * 0.92},${s * 0.88} ${s * 0.08},${s * 0.88}`}
            fill={fill}
            stroke={stroke}
            strokeWidth={sw}
          />
    ...
    if (shape.id === 'ph_sq')
      return (
        <svg viewBox={`0 0 ${s} ${s}`} width={s} height={s}>
          <rect x={s * 0.1} y={s * 0.1} width={s * 0.8} height={s * 0.8} fill={fill} stroke={stroke} strokeWidth={sw} />
        </svg>
      )
    if (shape.id === 'ph_rect')
      return (
    ...
    return (
      <svg viewBox={`0 0 ${s} ${s}`} width={s} height={s}>
        <polygon
          points={`${s * 0.28},${s * 0.25} ${s * 0.72},${s * 0.25} ${s * 0.9},${s * 0.78} ${s * 0.1},${s * 0.78}`}
          fill={fill}
          stroke={stroke}
          strokeWidth={sw}
        />
  ...
  return (
    <svg viewBox={`0 0 ${s} ${s}`} width={s} height={s}>
      <path d={ptsToPath(pts)} fill={fill} stroke={stroke} strokeWidth={sw} strokeLinejoin="round" />
    </svg>
  )
}
    ...
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '40px 20px' }}>
      ...
      <Card style={{ marginBottom: 24 }}>
            ...
            <div key={k} style={{ marginBottom: 18 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                  ...
                  <span style={{ color: barCol, fontWeight: 'bold', fontSize: 14, minWidth: 42, textAlign: 'right' }}>
                    {p}%
                  </span>
                  <span style={{ color: T.muted, fontSize: 12, minWidth: 52, textAlign: 'right' }}>
                    {pts}/{WEIGHTS[k]} Pkt.
                  </span>
                </div>
              </div>
        ...
        >
          ...
          <span style={{ color: col, fontWeight: 'bold', fontSize: 18 }}>
            {total40.toFixed(1)} / 40 Punkten = {totalPct}%
          </span>
        </div>
      </Card>
      ...
      <div style={{ textAlign: 'center', color: T.muted, fontSize: 11 }}>
        Esc → Hauptmenü · Enter → Antworten ansehen
      </div>
    </div>
  )
}
...
function PhaseCard({ phase, onStart, timeLeft }) {
  ...
  return (
    <div style={{ maxWidth: 540, margin: '80px auto', padding: '24px 20px', textAlign: 'center' }}>
      <div style={{ color: T.muted, fontSize: 12, letterSpacing: 3, marginBottom: 16 }}>
        NÄCHSTER ABSCHNITT ({idx + 1}/6)
      </div>
      ...
      {phase.n > 0 && (
        <div style={{ color: T.muted, fontSize: 15, marginBottom: 24 }}>
          {phase.n} Fragen · {Math.round(phase.secs / 60)} Minuten
        </div>
      )}
      {phase.id === 'allerg_l' && (
        <div style={{ color: T.muted, fontSize: 15, marginBottom: 24 }}>
          8 Ausweise · {Math.round(phase.secs / 60)} Minuten Merkzeit
        </div>
      )}
      <button
        onClick={onStart}
...
function phWeight(id) {
  const w = PH_WEIGHTS[id]
  return w ? w + ' / 40' : ''
}
...
function describeSavedSim(s) {
  ...
  const counts = [answered('fig'), answered('zahlen'), answered('wort'), answered('allerg'), answered('impl')].filter(
    Boolean
  )
  const phaseMap = {
    allerg_l: 'Merkphase',
    figuren: 'Figuren',
    zahlen: 'Zahlenfolgen',
    wort: 'Wortflüssigkeit',
    allerg_q: 'Allergie-Abfrage',
    impl: 'Implikationen',
    results: 'Ergebnisse',
    phase_card: 'Bereit',
  }
...
export default function Simulation({ onBack }) {
  ...
  function score(answers, questions) {
    const correct = answers.reduce((s, a, i) => s + (a === questions[i].correctIdx ? 1 : 0), 0)
  ...
  async function startPhase() {
    ...
    if (ph.id === 'figuren') {
      ...
      setFigQuestions(qs)
      setFigAnswers(Array(15).fill(null))
      setSimPhase('figuren')
  ...
  // ── Render ────────────────────────────────────────────────────────────────
  const simSkRows = []
  const { isStartFocused: simSkS } = useSettingsKeyboard(
    simSkRows,
    () => {
      setPhaseIdx(0)
      setSimPhase('phase_card')
        ...
        <Card>
          <div style={{ marginBottom: 24 }}>
            <div style={{ color: T.muted, fontSize: 13, marginBottom: 16 }}>
              Simulierter MedAT-Testtag in Originalreihenfolge. Innerhalb jeder Kategorie kannst du Fragen überspringen
              und später beantworten. Ergebnisse werden erst am Ende angezeigt.
            </div>
            <div
              style={{
                display: 'flex',
              ...
              {PHASES.map((ph, i) => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ color: T.muted, fontSize: 12 }}>
                      {ph.id === 'allerg_l' ? '8 Ausweise lernen' : `${ph.n} Fragen`} · {Math.round(ph.secs / 60)} Min
                    </span>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <button
              ...
              style={{
                ...
                fontSize: 16,
                fontWeight: 'bold',
                boxShadow: simSkS() ? `0 0 0 3px ${T.orange}88` : 'none',
              }}
            >
              Starten
            </button>
          ...
          <div style={{ background: T.surf2, borderRadius: 6, height: 6, marginBottom: 24 }}>
            <div
              style={{
                ...
                background: T.green,
                borderRadius: 6,
                width: `${(timer / PHASES.find((p) => p.id === 'allerg_l').secs) * 100}%`,
      ...
      <div>
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '20px 20px' }}>
          <ZahlenQuiz
            ...
            answers={zahlenAnswers}
            onAnswer={(qi, i) => {
              const a = [...zahlenAnswers]
              a[qi] = a[qi] === i ? null : i
              setZahlenAnswers(a)
            }}
            color={T.blue}
    ...
    return (
      <div>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '20px 20px' }}>
          <AllergieQuiz
            questions={allergQuestions}
            answers={allergAnswers}
            onAnswer={(qi, i) => {
              const a = [...allergAnswers]
              a[qi] = a[qi] === i ? null : i
              setAllergAnswers(a)
            }}
            color={T.green}
