...
export function makeTask(dir) {
  ...
  if (d === 'numberToWord') {
    ...
    return {
      category: 'numbers',
      direction: 'wordToNumber',
      number: target,
      correctWord: MAJOR[target],
      options,
      correctIdx: options.indexOf(correctNumber),
    }
  }
}
...
export function makeDigitTask(dir) {
  ...
  if (d === 'digitToWord') {
    const distractors = shuffle(DIGITS.filter((_, i) => i !== target))
      .slice(0, 4)
      .map((x) => x.word)
    const options = shuffle([digit.word, ...distractors])
    return {
      category: 'digits',
      direction: 'digitToWord',
      digit: target,
      consonants: digit.consonants,
      correctWord: digit.word,
      options,
      correctIdx: options.indexOf(digit.word),
    }
  } else {
    const correctDigit = String(target)
    const distractors = shuffle(Array.from({ length: 9 }, (_, i) => String(i + (i >= target ? 1 : 0)))).slice(0, 4)
    const options = shuffle([correctDigit, ...distractors])
    return {
      category: 'digits',
      direction: 'wordToDigit',
      digit: target,
      consonants: digit.consonants,
      correctWord: digit.word,
      options,
      correctIdx: options.indexOf(correctDigit),
    }
  }
}
...
export default function MajorSystem({ onBack }) {
  ...
  const answer = useCallback(
    (i) => {
      ...
      setTimeout(() => {
        setShowFb(true)
        setTimeout(() => setFbReady(true), 250)
      }, 50)
    },
  ...
  function submitTyped() {
    ...
    const correct = showNum
      ...
      : question.category === 'digits'
        ? String(question.digit)
        : String(question.number)
    const isCorrect = showNum ? raw.toLowerCase() === correct.toLowerCase() : raw === correct
  ...
  const catRow = CATEGORIES.map((c) => ({
    action: () => {
      setCategory(c.v)
      setDir(isNumbers ? 'numberToWord' : 'digitToWord')
    },
  }))
    ...
    return (
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div style={{ textAlign: 'center', marginTop: 16 }}>
          <a
            href="https://de.wikipedia.org/wiki/Major-System"
            target="_blank"
            rel="noreferrer"
  ...
  const promptText = showNum
    ? isDigit
      ? 'Welches Bildwort gehört zu dieser Ziffer?'
      : 'Welches Bildwort gehört zu dieser Zahl?'
    : isDigit
      ? 'Zu welcher Ziffer gehört dieses Bildwort?'
      : 'Zu welcher Zahl gehört dieses Bildwort?'
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
      <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
        ...
        <span style={{ color: T.muted, fontSize: 13 }}>
          {isDigit
            ? `Ziffern · ${showNum ? 'Ziffer→Wort' : 'Wort→Ziffer'}`
            : `Zahlen · ${showNum ? 'Zahl→Wort' : 'Wort→Zahl'}`}
      ...
      <div style={{ color: T.muted, fontSize: 13, marginBottom: 4 }}>
        {isTyping ? (
          typedCorrect ? (
            ...
            <span>
              Deine Eingabe: <span style={{ color: T.red }}>{inputVal}</span> · Richtig:{' '}
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <input
              ...
              style={{
                flex: 1,
                background: T.surf2,
                border: `1px solid ${showFb ? (typedCorrect ? T.green : T.red) : T.border}`,
                borderRadius: 8,
                color: T.text,
                padding: '14px 18px',
                fontSize: 20,
