...
function genCircPieces(sweep) {
  ...
  const arcPoly = isFullCircle
    ? Array.from({ length: 64 }, (_, i) => {
        const a = a0 + (2 * Math.PI * i) / 64
        return [cx + r * Math.cos(a), cy + r * Math.sin(a)]
      })
    : [
        ...Array.from({ length: 48 }, (_, i) => {
          const a = a0 + (sweep * i) / 47
          return [cx + r * Math.cos(a), cy + r * Math.sin(a)]
        }),
        [cx, cy],
      ]
    ...
    if (shape.id === 'ph_tri')
      return (
        <svg viewBox={`0 0 ${s} ${s}`} width={s} height={s}>
          <polygon
            points={`${cx},${s * 0.1} ${s * 0.92},${s * 0.9} ${s * 0.08},${s * 0.9}`}
            fill={fill}
            stroke={stroke}
            strokeWidth={sw}
          />
...
function genRotations(pieces) {
  return pieces.map(() => {
    ...
    if (r < 0.3) return rnd(325, 352)
    if (r < 0.55) return rnd(40, 140)
    if (r < 0.8) return rnd(220, 320)
    return rnd(145, 215)
  })
}
...
function WinkelDisplay({ deg, display = {}, size = 200 }) {
  ...
  const halfRad = rad / 2
  const x0 = cx + rayLen * Math.sin(halfRad),
    y0 = cy - rayLen * Math.cos(halfRad)
  const x1 = cx - rayLen * Math.sin(halfRad),
    y1 = cy - rayLen * Math.cos(halfRad)
  return (
...
export function makeTask() {
  ...
  const pd = targetShape.family === 'polygon' ? genPolyPieces(targetShape.sides) : genCircPieces(targetShape.sweep)
  const others = shuffle(family.filter((s) => s.id !== targetShape.id))
  const injectNone = Math.random() < 0.2
...
export function FigurenQuiz({ questions, answers, onAnswer, color }) {
  ...
  const rotations = useMemo(() => questions.map((q) => genRotations(q.pieces.normalized || [])), [questions])
                ...
                {q.opts.map((o, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {o.shape ? (
                        <>
                          <AnswerSVG shape={o.shape} size={56} />
                          <span>{o.shape.label}</span>
                        </>
                      ) : (
                        <span style={{ color: T.muted }}>Keine der Figuren ist richtig.</span>
                      )}
...
export default function Figuren({ onBack }) {
  ...
  function figEndlessAnswer(i) {
    ...
    setTimeout(() => {
      setFigShowFb(true)
      setTimeout(() => setFigFbReady(true), 200)
    }, 80)
  }
  ...
  function winkelAnswer(i) {
    ...
    setSelected(i)
    if (correct) setWinkelScore((s) => s + 1)
    setWinkelTotal((t) => t + 1)
    if (endless)
      setTimeout(() => {
        setShowFb(true)
  ...
  useEffect(() => {
    if (fbReady) {
      const h = (e) => {
        if (e.key === 'Escape') {
          endless ? setDone(true) : setMode('settings')
          return
        }
        winkelNextQ()
      }
    ...
    if (!showFb && mode === 'winkel') {
      const h = (e) => {
        ...
        if (!endless && selected !== null) {
          clearTimeout(advanceRef.current)
          winkelNextQ()
          return
        }
        const i = KEYS.indexOf(e.key.toLowerCase())
        if (i >= 0 && i < 5) winkelAnswer(i)
      }
      window.addEventListener('keydown', h)
      return () => window.removeEventListener('keydown', h)
    }
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <Card>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {[
                ...
                <button
                  ...
                  style={{
                    background: count === o.v ? `${T.teal}22` : T.surf2,
                    border: `1px solid ${count === o.v ? T.teal : T.border}`,
                    borderRadius: 8,
                    color: count === o.v ? T.teal : T.text,
                    cursor: 'pointer',
                    padding: '8px 18px',
                    fontSize: 14,
                    boxShadow: skF(0, i) ? `0 0 0 2px ${T.teal}` : 'none',
                  }}
                >
                  {o.l}
                </button>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {[
                { v: 'figuren', l: 'Figuren zusammensetzen' },
                { v: 'winkel', l: 'Winkel üben' },
              ].map((o, i) => (
                <button
                  key={o.v}
                  onClick={() => setQuizType(o.v)}
                  style={{
                    background: quizType === o.v ? `${T.teal}22` : T.surf2,
                    border: `1px solid ${quizType === o.v ? T.teal : T.border}`,
                    borderRadius: 8,
                    color: quizType === o.v ? T.teal : T.text,
                    cursor: 'pointer',
                    padding: '8px 18px',
                    fontSize: 14,
                    boxShadow: skF(1, i) ? `0 0 0 2px ${T.teal}` : 'none',
                  }}
                >
                  {o.l}
                </button>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 16,
              fontWeight: 'bold',
              boxShadow: skS() ? `0 0 0 3px ${T.teal}88` : 'none',
            }}
          >
            Starten
          </button>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  borderRadius: 10,
                  color: T.text,
                  cursor: figSelected !== null ? 'default' : 'pointer',
                  padding: '10px 16px',
                  fontSize: 14,
                  marginBottom: 8,
                  transition: 'all 0.15s',
                ...
                {figSelected === q.correctIdx ? (
                  ...
                  <span>
                    ...
                    <span style={{ color: T.green, fontWeight: 'bold' }}>
                      {q.opts[q.correctIdx].shape ? q.opts[q.correctIdx].shape.label : 'Keine der Figuren ist richtig.'}
                    </span>
                  </span>
                )}
              </div>
    ...
    return (
      <div style={{ maxWidth: 840, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div style={{ marginTop: 24, textAlign: 'center' }}>
          ...
          >
            Ergebnis anzeigen ({figTot}/{questions.length})
          </button>
        </div>
      </div>
    )
  ...
  const btnStyle = (i) => ({
    ...
    width: '100%',
    background:
      selected === null ? T.surf2 : i === q.correctIdx ? `${T.green}22` : i === selected ? `${T.red}22` : T.surf2,
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          <button
            onClick={() => (endless ? setDone(true) : setMode('settings'))}
            style={{
              background: 'none',
              border: `1px solid ${T.border}`,
              borderRadius: 8,
      ...
      <Card style={{ marginBottom: 16 }}>
        <div style={{ color: T.muted, fontSize: 13, marginBottom: 16 }}>
          Zu welchem regelmäßigen Vieleck gehört dieser Winkel?
        </div>
      ...
      <Card>
        {q.opts.map((o, i) => (
          <button key={i} onClick={() => winkelAnswer(i)} style={btnStyle(i)}>
            ...
            {selected !== null && i === selected && i !== q.correctIdx && (
              <span style={{ color: T.red, marginLeft: 'auto' }}>✗</span>
            )}
          </button>
        ))}
          ...
          <div style={{ marginTop: 16, background: T.surf2, borderRadius: 12, padding: '16px 20px' }}>
            <div style={{ fontSize: 14, marginBottom: 6 }}>
              <span style={{ color: T.muted }}>
                Dieser Winkel ({q.deg}°) ist der {q.type === 'innen' ? 'Innenwinkel' : 'Spitzenwinkel'} eines{' '}
              </span>
                ...
                {wInnen.map((w) => (
                  <span
                    ...
                    style={{
                      ...
                      padding: '3px 9px',
                      borderRadius: 6,
                      background: w.deg === q.deg ? `${T.teal}22` : T.bg,
                      color: w.deg === q.deg ? T.teal : T.muted,
                      border: `1px solid ${w.deg === q.deg ? T.teal : T.border}`,
                    }}
                  >
                    {w.label}: {w.deg}°
                  </span>
