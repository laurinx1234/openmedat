...
export async function fetchPhotos(count=8){
  try{
    const res=await fetch(`https://randomuser.me/api/?results=${count}&inc=picture&noinfo`,{signal:AbortSignal.timeout?AbortSignal.timeout(8000):undefined})
...
export function AusweisCard({card}){
  // Field order: name, geburtstag, medikamente, blutgruppe, allergien, ausweisnummer, land
  const rows=[['Geburtstag',card.geburtstag],['Medikamenteneinnahme',card.medikamente],['Blutgruppe',card.blutgruppe],['Allergien',card.allergien],['Ausweis-Nr.',card.ausweisnummer],['Ausstellungsland',card.land]]
  return<div style={{background:T.surf,border:`1px solid ${T.border}`,borderRadius:12,padding:14,width:'100%'}}>
    ...
    <div style={{display:'grid',gap:3}}>
      {rows.map(([k,v])=>(<div key={k} style={{display:'flex',gap:8,fontSize:12}}><span style={{color:T.muted,minWidth:130,flexShrink:0}}>{k}:</span><span style={{color:T.text,wordBreak:'break-word'}}>{v}</span></div>))}
    </div>
  </div>
}
...
export default function Allergieausweise({onBack}){
  ...
  const[settings,setSettings]=useState({cardCount:4,learnMin:15,qCount:10})
  ...
  async function startLearn(){
    ...
    setAllCards(pool);setShownCards(pool.slice(0,settings.cardCount))
    resetLearn(settings.learnMin*60);setPhase('learn')
  }

  useEffect(()=>{
    if(phase==='learn'&&learnTimer<=0){
      const qs=Array.from({length:settings.qCount},()=>makeQuestion(shownCards,allCards))
      setQuestions(qs);setQIdx(0);setSelected(null);setScore(0);setDone(false);setPhase('quiz')
    }
  },[learnTimer,phase])
  ...
  if(done)return(<div style={{maxWidth:680,margin:'0 auto',padding:'24px 20px'}}><BackBtn onBack={onBack}/><ResultScreen correct={score} total={questions.length} onRetry={()=>setPhase('settings')} onBack={onBack}/></div>)
