...
function genName(used) {
  ...
  for (let _ = 0; _ < 300; _++) {
    let nm = ''
    for (let i = 0; i < LEN; i++) nm += String.fromCharCode(65 + rnd(0, 25))
    if (!used.has(nm)) {
      used.add(nm)
      return nm
    }
...
function genBirthday() {
  const m = rnd(0, 11)
  return `${rnd(1, DAYS[m])}. ${MONTHS[m]}`
}

export function genCardPool() {
  const usedNames = new Set()
  return Array.from({ length: 8 }, (_, i) => ({
    name: genName(usedNames),
    geburtstag: genBirthday(),
    medikamente: Math.random() > 0.5 ? 'Ja' : 'Nein',
    blutgruppe: pick(BLUTGRUPPEN),
    allergien: shuffle([...ALLERGENE])
      .slice(0, rnd(1, 3))
      .join(', '),
    ausweisnummer: String(rnd(10000, 99999)),
    land: pick(LAENDER),
    photoUrl: null,
  }))
}
...
export async function fetchPhotos(count = 8) {
  try {
    ...
    return Array.from({ length: count }, (_, i) => `https://i.pravatar.cc/150?img=${i + 10}`)
  }
}

export function AusweisCard({ card }) {
  return (
    <div style={{ background: T.surf, border: `1px solid ${T.border}`, borderRadius: 12, padding: 14, width: '100%' }}>
      <div style={{ display: 'flex', gap: 12, marginBottom: 10 }}>
        <img
          ...
          height={70}
          alt={card.name}
          style={{ borderRadius: '50%', objectFit: 'cover', objectPosition: 'center top', flexShrink: 0 }}
        />
      ...
      <div style={{ display: 'grid', gap: 3 }}>
        {[
          ['Geburtstag', card.geburtstag],
          ['Medikamenteneinnahme', card.medikamente],
          ['Blutgruppe', card.blutgruppe],
          ['Allergien', card.allergien],
          ['Ausweis-Nr.', card.ausweisnummer],
          ['Ausstellungsland', card.land],
        ].map(([k, v]) => (
          <div key={k} style={{ display: 'flex', gap: 8, fontSize: 12 }}>
            <span style={{ color: T.muted, minWidth: 130, flexShrink: 0 }}>{k}:</span>
            <span style={{ color: T.text, wordBreak: 'break-word' }}>{v}</span>
          </div>
        ))}
      </div>
    </div>
        ...
        {questions.map((q, qi) => {
          ...
          return (
            <div
              ...
              style={{
                borderRadius: 12,
                outline: isFocused ? `2px solid ${T.blue}` : '2px solid transparent',
                outlineOffset: 2,
            ...
            >
              <Card>
                <div
                  style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: q.showAvatar ? 12 : 8 }}
                >
                ...
                {q.opts.map((o, i) => (
                  <OptionBtn
                    ...
                    text={o === 'keine' ? 'Keine Antwort ist richtig.' : o}
                  />
                ))}
              </Card>
            </div>
...
const Q_TYPES = [
  // Name → Attribut
  {
    label: (c) => `Wie lautet die Ausweisnummer von ${c.name}?`,
    correct: (c) => c.ausweisnummer,
    pool: (cs) => cs.map((c) => c.ausweisnummer),
    av: false,
  },
  {
    label: (c) => `Welche Blutgruppe hat ${c.name}?`,
    correct: (c) => c.blutgruppe,
    pool: (cs) => cs.map((c) => c.blutgruppe),
    av: false,
  },
  {
    label: (c) => `Welche Allergie/n hat ${c.name}?`,
    correct: (c) => c.allergien,
    pool: (cs) => cs.map((c) => c.allergien),
    av: false,
  },
  {
    label: (c) => `Wann hat ${c.name} Geburtstag?`,
    correct: (c) => c.geburtstag,
    pool: (cs) => cs.map((c) => c.geburtstag),
    av: false,
  },
  {
    label: (c) => `Aus welchem Land stammt der Ausweis von ${c.name}?`,
    correct: (c) => c.land,
    pool: (cs) => cs.map((c) => c.land),
    av: false,
  },
  {
    label: (c) => `Welche Beschreibung passt am besten zu ${c.name}?`,
    correct: (c) => `Blutgruppe: ${c.blutgruppe}, Medikamente: ${c.medikamente}`,
    pool: (cs) => cs.map((c) => `Blutgruppe: ${c.blutgruppe}, Medikamente: ${c.medikamente}`),
    av: false,
  },
  // Ausweis-Nr. → Attribut
  {
    label: (c) => `Wie heißt die Person mit der Ausweisnummer ${c.ausweisnummer}?`,
    correct: (c) => c.name,
    pool: (cs) => cs.map((c) => c.name),
    av: false,
  },
  {
    label: (c) => `In welchem Land wurde Ausweis-Nr. ${c.ausweisnummer} ausgestellt?`,
    correct: (c) => c.land,
    pool: (cs) => cs.map((c) => c.land),
    av: false,
  },
  {
    label: (c) => `Welche Blutgruppe hat die Person mit Ausweis-Nr. ${c.ausweisnummer}?`,
    correct: (c) => c.blutgruppe,
    pool: (cs) => cs.map((c) => c.blutgruppe),
    av: false,
  },
  {
    label: (c) => `Welche Allergie/n hat die Person mit Ausweis-Nr. ${c.ausweisnummer}?`,
    correct: (c) => c.allergien,
    pool: (cs) => cs.map((c) => c.allergien),
    av: false,
  },
  {
    label: (c) => `Welche Beschreibung passt am besten zur Person mit Ausweis-Nr. ${c.ausweisnummer}?`,
    correct: (c) => `Blutgruppe: ${c.blutgruppe}, Medikamente: ${c.medikamente}`,
    pool: (cs) => cs.map((c) => `Blutgruppe: ${c.blutgruppe}, Medikamente: ${c.medikamente}`),
    av: false,
  },
  // Geburtstag → Attribut
  {
    label: (c) => `Wie heißt die Person, die am ${c.geburtstag} Geburtstag feiert?`,
    correct: (c) => c.name,
    pool: (cs) => cs.map((c) => c.name),
    av: false,
  },
  {
    label: (c) => `Welche Ausweisnummer hat die Person, die am ${c.geburtstag} Geburtstag feiert?`,
    correct: (c) => c.ausweisnummer,
    pool: (cs) => cs.map((c) => c.ausweisnummer),
    av: false,
  },
  {
    label: (c) => `Welche Blutgruppe hat die Person, die am ${c.geburtstag} Geburtstag feiert?`,
    correct: (c) => c.blutgruppe,
    pool: (cs) => cs.map((c) => c.blutgruppe),
    av: false,
  },
  {
    label: (c) => `Welche Allergien hat die Person, die am ${c.geburtstag} Geburtstag feiert?`,
    correct: (c) => c.allergien,
    pool: (cs) => cs.map((c) => c.allergien),
    av: false,
  },
  {
    label: (c) => `Aus welchem Land stammt der Ausweis der Person, die am ${c.geburtstag} Geburtstag feiert?`,
    correct: (c) => c.land,
    pool: (cs) => cs.map((c) => c.land),
    av: false,
  },
  {
    label: (c) => `Welche Beschreibung passt am besten zur Person, die am ${c.geburtstag} Geburtstag feiert?`,
    correct: (c) => `Blutgruppe: ${c.blutgruppe}, Medikamente: ${c.medikamente}`,
    pool: (cs) => cs.map((c) => `Blutgruppe: ${c.blutgruppe}, Medikamente: ${c.medikamente}`),
    av: false,
  },
  // Allergie → Attribut
  {
    label: (c) => `Wie heißt die Person mit der ${c.allergien} Allergie?`,
    correct: (c) => c.name,
    pool: (cs) => cs.map((c) => c.name),
    av: false,
  },
  {
    label: (c) => `Welche Ausweisnummer hat die Person mit der ${c.allergien} Allergie?`,
    correct: (c) => c.ausweisnummer,
    pool: (cs) => cs.map((c) => c.ausweisnummer),
    av: false,
  },
  {
    label: (c) => `Welche Blutgruppe hat die Person mit der ${c.allergien} Allergie?`,
    correct: (c) => c.blutgruppe,
    pool: (cs) => cs.map((c) => c.blutgruppe),
    av: false,
  },
  {
    label: (c) => `Wann hat die Person mit der ${c.allergien} Allergie Geburtstag?`,
    correct: (c) => c.geburtstag,
    pool: (cs) => cs.map((c) => c.geburtstag),
    av: false,
  },
  {
    label: (c) => `Aus welchem Land stammt der Ausweis der Person mit der ${c.allergien} Allergie?`,
    correct: (c) => c.land,
    pool: (cs) => cs.map((c) => c.land),
    av: false,
  },
  {
    label: (c) => `Welche Beschreibung passt am besten zur Person mit der ${c.allergien} Allergie?`,
    correct: (c) => `Blutgruppe: ${c.blutgruppe}, Medikamente: ${c.medikamente}`,
    pool: (cs) => cs.map((c) => `Blutgruppe: ${c.blutgruppe}, Medikamente: ${c.medikamente}`),
    av: false,
  },
  // Land → Attribut
  {
    label: (c) => `Wie heißt die Person, deren Ausweis aus ${c.land} stammt?`,
    correct: (c) => c.name,
    pool: (cs) => cs.map((c) => c.name),
    av: false,
  },
  {
    label: (c) => `Welche Ausweisnummer hat die Person, deren Ausweis aus ${c.land} stammt?`,
    correct: (c) => c.ausweisnummer,
    pool: (cs) => cs.map((c) => c.ausweisnummer),
    av: false,
  },
  {
    label: (c) => `Welche Blutgruppe hat die Person, deren Ausweis aus ${c.land} stammt?`,
    correct: (c) => c.blutgruppe,
    pool: (cs) => cs.map((c) => c.blutgruppe),
    av: false,
  },
  {
    label: (c) => `Welche Allergien hat die Person, deren Ausweis aus ${c.land} stammt?`,
    correct: (c) => c.allergien,
    pool: (cs) => cs.map((c) => c.allergien),
    av: false,
  },
  {
    label: (c) => `Wann hat die Person Geburtstag, deren Ausweis aus ${c.land} stammt?`,
    correct: (c) => c.geburtstag,
    pool: (cs) => cs.map((c) => c.geburtstag),
    av: false,
  },
  {
    label: (c) => `Welche Beschreibung passt am besten zur Person, deren Ausweis aus ${c.land} stammt?`,
    correct: (c) => `Blutgruppe: ${c.blutgruppe}, Medikamente: ${c.medikamente}`,
    pool: (cs) => cs.map((c) => `Blutgruppe: ${c.blutgruppe}, Medikamente: ${c.medikamente}`),
    av: false,
  },
  ...
  {
    label: (c) => `Wann hat die Person mit der Ausweisnummer ${c.ausweisnummer} Geburtstag?`,
    correct: (c) => c.geburtstag,
    pool: (cs) => cs.map((c) => c.geburtstag),
    av: false,
  },
  ...
  {
    label: (c) => `Wie heißt die Person mit der Zahl ${c.ausweisnummer[0]} an erster Stelle der Ausweisnummer?`,
    correct: (c) => c.name,
    pool: (cs) => cs.map((c) => c.name),
    av: false,
    check: (c, sh) => sh.filter((s) => s.ausweisnummer[0] === c.ausweisnummer[0]).length === 1,
  },
  {
    label: (c) => `Wie heißt die Person mit der Zahl ${c.ausweisnummer[4]} an letzter Stelle der Ausweisnummer?`,
    correct: (c) => c.name,
    pool: (cs) => cs.map((c) => c.name),
    av: false,
    check: (c, sh) => sh.filter((s) => s.ausweisnummer[4] === c.ausweisnummer[4]).length === 1,
  },
  // Foto-Fragen (av:true — kein Name in Frage, kein Name in Anzeige)
  {
    label: () => `Welche Allergie/n hat diese Person?`,
    correct: (c) => c.allergien,
    pool: (cs) => cs.map((c) => c.allergien),
    av: true,
  },
  {
    label: () => `Welche Blutgruppe hat diese Person?`,
    correct: (c) => c.blutgruppe,
    pool: (cs) => cs.map((c) => c.blutgruppe),
    av: true,
  },
  ...
  {
    label: () => `Aus welchem Land stammt der Ausweis dieser Person?`,
    correct: (c) => c.land,
    pool: (cs) => cs.map((c) => c.land),
    av: true,
  },
  {
    label: () => `Wann hat diese Person Geburtstag?`,
    correct: (c) => c.geburtstag,
    pool: (cs) => cs.map((c) => c.geburtstag),
    av: true,
  },
  {
    label: () => `Wie lautet die Ausweisnummer dieser Person?`,
    correct: (c) => c.ausweisnummer,
    pool: (cs) => cs.map((c) => c.ausweisnummer),
    av: true,
  },
  {
    label: () => `Welche Beschreibung passt am besten zu dieser Person?`,
    correct: (c) => `Blutgruppe: ${c.blutgruppe}, Medikamente: ${c.medikamente}`,
    pool: (cs) => cs.map((c) => `Blutgruppe: ${c.blutgruppe}, Medikamente: ${c.medikamente}`),
    av: true,
  },
...
function tryMultiQuestion(shown) {
  ...
  const question = useNrs
    ? `Welche Ausweisnummern haben Personen mit der Blutgruppe ${bg}?`
    : `Wer hat die Blutgruppe ${bg}?`
...
export function makeQuestion(shown, all) {
  if (Math.random() < 0.25) {
    const mq = tryMultiQuestion(shown)
    if (mq) return mq
  }
  for (let _ = 0; _ < 40; _++) {
    const card = pick(shown)
    const qt = pick(Q_TYPES)
    ...
    let opts, ci
    if (noneCorrect) {
      const ws = shuffle([...pool]).slice(0, 4)
...
export default function Allergieausweise({ onBack }) {
  ...
  const [settings, setSettings] = useState(
    () => sInit?.settings || { cardCount: 8, learnMin: 8, quizDelayMin: 35, qCount: 25 }
  )
  ...
  useEffect(() => {
    setSettings((s) => ({ ...s, learnMin: s.cardCount }))
  }, [settings.cardCount])
  ...
  async function startLearn() {
    ...
    photos.forEach((url, i) => {
      if (i < pool.length) pool[i].photoUrl = url
    })
    setAllCards(pool)
  ...
  const skGroupDefs = [
    [{ v: 2 }, { v: 3 }, { v: 4 }, { v: 5 }, { v: 6 }, { v: 7 }, { v: 8 }]
      .map(() => ({ action: () => {} }))
      .map((o, i) => ({ action: () => setSettings((s) => ({ ...s, cardCount: [2, 3, 4, 5, 6, 7, 8][i] })) })),
    Array.from({ length: 20 }, (_, i) => ({ action: () => setSettings((s) => ({ ...s, learnMin: i + 1 })) })),
    [5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60].map((v, i) => ({
      action: () => setSettings((s) => ({ ...s, quizDelayMin: v })),
    })),
    [5, 10, 15, 20, 25].map((v) => ({ action: () => setSettings((s) => ({ ...s, qCount: v })) })),
  ]
  const { isFocused: skF, isStartFocused: skS } = useSettingsKeyboard(
    skGroupDefs,
    startLearn,
    onBack,
    phase === 'settings'
          ...
          >
            <div>
              ...
              <div style={{ color: T.muted, fontSize: 13 }}>
                Die Zeit läuft. Quiz startet {isQuizReady() ? 'jetzt' : 'nach der Merkzeit'}.
              </div>
            </div>
            <button
              onClick={startQuizFromStore}
              style={{
                background: T.green,
                border: 'none',
                borderRadius: 8,
        ...
        <Card>
          {[
            ...
            { label: 'Lernzeit', key: 'learnMin', type: 'slider', min: 1, max: 20, row: 1 },
            {
              label: 'Wartezeit',
              key: 'quizDelayMin',
              opts: [5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60].map((v) => ({ v, l: v + 'm' })),
              row: 2,
            },
            {
              label: 'Anzahl Fragen',
              ...
              {type === 'slider' ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      height: 6,
                      cursor: 'pointer',
                      boxShadow: skF(row, 0) ? `0 0 0 3px ${T.green}88` : 'none',
                      borderRadius: 4,
                      outline: 'none',
                    }}
                  />
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {opts.map((o, i) => (
                    <button
                      key={o.v}
                      onClick={() => setSettings((s) => ({ ...s, [key]: o.v }))}
                      style={{
                        background: settings[key] === o.v ? `${T.green}22` : T.surf2,
                        border: `1px solid ${settings[key] === o.v ? T.green : T.border}`,
                        borderRadius: 8,
                        color: settings[key] === o.v ? T.green : T.text,
                        cursor: 'pointer',
                        padding: '8px 14px',
                        fontSize: 13,
                        boxShadow: skF(row, i) ? `0 0 0 2px ${T.green}` : 'none',
                      }}
                    >
                      {o.l}
                    </button>
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 'bold',
              marginTop: 8,
              boxShadow: skS() ? `0 0 0 3px ${T.green}88` : 'none',
            }}
          >
            Fotos laden & starten
          </button>
  ...
  if (phase === 'learn')
    return (
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div style={{ background: T.surf2, borderRadius: 6, height: 6, marginBottom: 24 }}>
          <div
            style={{
              height: '100%',
              background: T.green,
              borderRadius: 6,
              width: `${(learnTimer / (settings.learnMin * 60)) * 100}%`,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: 16 }}>
          {shownCards.map((c, i) => (
            <AusweisCard key={i} card={c} />
          ))}
        </div>
        <div style={{ color: T.muted, fontSize: 12, marginTop: 16, textAlign: 'center' }}>
          Esc oder "Fertig" → Quiz-Timer starten, andere Übungen machen
        </div>
      </div>
    )
  ...
  if (phase === 'learn_done') {
    ...
    return (
      <div style={{ maxWidth: 600, margin: '80px auto', padding: '24px 20px', textAlign: 'center' }}>
        ...
        {ready ? (
          <>
            <div style={{ color: T.text, fontSize: 16, marginBottom: 32 }}>
              Die Abfragezeit ist abgelaufen. Jetzt Quiz starten!
            </div>
            <button
              onClick={startQuiz}
              style={{
          ...
          <>
            <div style={{ color: T.text, fontSize: 15, marginBottom: 8 }}>
              Du hast noch Zeit. Mach andere Übungen und komm zurück!
            </div>
            <div style={{ color: T.muted, fontSize: 13, marginBottom: 32 }}>
              Quiz wird nach {settings.quizDelayMin} Minuten Abfragezeit freigeschaltet.
            </div>
            <button
              onClick={() => navigate('/')}
              style={{
    ...
    return (
      <div style={{ maxWidth: 780, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div style={{ textAlign: 'center', padding: '20px 0 32px' }}>
          ...
          <div style={{ color: T.muted, fontSize: 16 }}>
                    ...
                    <div style={{ fontSize: 13 }}>
                      ...
                      {!isCorrect && (
                        <div style={{ color: T.green }}>
                          Richtig:{' '}
                          {q.opts[q.correctIdx] === 'keine' ? 'Keine Antwort ist richtig.' : q.opts[q.correctIdx]}
                        </div>
                      )}
                    </div>
                  </div>
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        >
          Ergebnis ({answeredCount}/{questions.length})
        </button>
      </div>
      ...
      <div style={{ marginTop: 24, display: 'flex', justifyContent: 'center' }}>
        ...
        >
          Ergebnis anzeigen ({answeredCount}/{questions.length})
        </button>
      </div>
      <KeyHint />
      <div style={{ color: T.muted, fontSize: 11, marginTop: 4, textAlign: 'center' }}>
        Tab ↦ nächste Frage · ↑↓ Antwort wählen · A–E antworten
      </div>
    </div>
  )
}
      ...
      if (isQuizReady()) {
        if (!beeped.current) {
          ...
          if ('Notification' in window && Notification.permission === 'granted') {
            new Notification('Bereit zur Abfrage', {
              body: 'Die Wartezeit ist abgelaufen. Du kannst jetzt die Fragen beantworten.',
            })
          }
        }
        onReadyRef.current?.()
